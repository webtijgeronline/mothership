...
function ship(ctx, W, H) {
  ...
  ctx.closePath()
  ctx.fillStyle = 'rgba(10,20,15,0.9)'; ctx.fill()
  ...
  [0.25,0.45,0.65,0.8].forEach(function(t){
    var lx=W*0.5-(W*0.45%)t*0.95, rx=W*0.5+(W*0.45)*t*0.95, y=H*0.02+H*0.9*t
    ctx.beginPath(); ctx.moveTo(lx,y); ctx.lineTo(rx,y); ctx.stroke()
  })
  [[W*0.2,H*0.955,14],[W*0.38,H*0.91,8],[W*0.5,H*0.935,10],[W*0.62,H*0.91,8],[W*0.8,H*0.955,14]].forEach(function(e){
...
export default function GameCanvas({ rooms, onToggle }) {
  ...
  useEffect(function() {
    ...
    function draw() {
      ...
      for(var i=0;i<60;i++){ctx.fillRect(((i*137.5)%100)/100*W,((i*97.3)%100)/100*H,(i%3===0)?1.2:0.6,(i%3===0)?1.2:0.6)}
      ...
      ROOMS.forEach(function(rd) {
        ...
        if(active){
          ...
          var cs=Math.min(rw,rh)*0.18
          [[rx,ry],[rx+rw,ry],[rx,ry+rh],[rx+rw,ry+rh]].forEach(function(p){
            var sx=p[0]===rx?1:-1, sy=p[1]===ry?1:-1
            ctx.beginPath(); ctx.moveTo(p[0]+sx*cs,p[1]); ctx.lineTo(p[0],p[1]); ctx.lineTo(p[0],p[1]+sy*cs); ctx.stroke()
          })
        ...
        if(active){
          ...
          var bw=rw-12; ctx.fillStyle='rgba('+c+',0.1)'; ctx.fillRect(rx+6,ry+rh-5,bw,3)
          ctx.fillStyle=rd.color; ctx.fillRect(rx+6,ry+rh-5,(Date.now()/25)%bw,3)
        }
        ...
        if(active){
          ...
          var ag2=agents.current[rd.id]; ag2.x=0.5; ag2.y=0.5; ag2.trail=[]
        }
      })

      var active2=roomsRef.current.filter(function(r){return r.active}), tot=active2.reduce(function(s,r){return s+(r.rev||0)},0)
      ...
      ctx.fillStyle='rgba(63,255,162,0.5)'; ctx.font=Math.max(7,Math.round(W*0.018))+'px Share Tech Mono,monospace'; ctx.fillText(active2.length+'/6 ONLINE',W*0.97,H*0.078); ctx.textAlign='left'
    ...
    function handleClick(e){
      ...
      ROOMS.forEach(function(rd){
        var r={x:rd.x*rect.width,y:rd.y*rect.height+rect.height*0.085,w:rd.w*rect.width,h:rd.h*rect.height}
